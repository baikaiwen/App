...
public class NetService {
  ...
  public void getFristData(final Context mContext,String code, final Handler handler) {
    ...
    String url =DbIp.getInstance(mContext).selectById()
        + "open?sql=select+a.f_zcd_date%2Ca.f_zc_dsp%2Ca.f_zcd_no"
        + "%2Ca.f_cus_no%2Cb.f_cus_dsp%2Ca.f_ywy+%2Cc.f_cus_dsp+as+f_ywy_dsp%0Afrom+un_zxdetail_ord+a+join+Customer+b+on+a.f_cus_no%3Db.f_cus_no++%0Ajoin+Customer+c+on+a.f_ywy%3Dc.f_cus_no++%0Awhere+a.f_txm%3D"
        + "'" + code + "'&token=&pagesize=5&pageindex=0";
    Log.i(TAG, "扫描配置单:" + url);
            ...
            try {
              ...
              if (Tools.judgeStringEquals(statu, "0")) {
                ...
                for (int i = 0; i < array.length(); i++) {
                  ...
                  ProductBean.getInstance().setBoothName(array.getJSONObject(i).getString("f_cus_dsp"));
                  ProductBean.getInstance().setCustmo(array.getJSONObject(i).getString("f_ywy"));
                  ProductBean.getInstance().setCustmoName(array.getJSONObject(i).getString("f_ywy_dsp"));
                }
                App.saveProduct(mContext, ProductBean.getInstance());
   ...
   * in(‘条码1’,’ 条码2’,’ 条码3’) ；其中f_bw_no是编码，f_qty是数量,f_bwdsp是品名
   * */
  public void reviewProduct(final Handler handler, String[] product, final Context mContext) {
    ...
    String url = DbIp.getInstance(mContext).selectById()
        + "open?sql=select+a.f_bw_no%2Cb.f_bw_dsp%2Csum(f_qty)+f_qty%2CSUM(f_weight)+f_weight+from+pbcpgbinfo+a+join+pbbwinfo+b+on+a"
        + ".f_bw_no%3Db.f_bw_no+%0Awhere+f_txm+in(" + productStr
        + ")++group+by+a.f_bw_no%2Cb.f_bw_dsp&token=&pagesize=5&pageindex=0";
    // String
    // url=StaticField.HOST+"open?sql=select+f_bw_no%2Csum(f_qty)+f_qty%2CSUM(f_weight)+f_weight+from+pbcpgbinfo+where+f_txm+in("
   ...
   * */
  public void insertZxDetails(Context mContext,final Handler handler, String cartonCode, String[] product, String[] proname,
      String[] pronum) {
    /* 拼接字符 条形码 */
    List<String> b = new ArrayList<String>();
    for (String str : product) {
   ...
   * 
   * */
  public void update(Context mContext,final Handler handler, String[] product) {
    /* 拼接字符 条形码 */
    List<String> b = new ArrayList<String>();
    for (String str : product) {
      b.add("'" + str + "',");
    ...
    String url =DbIp.getInstance(mContext).selectById()+ "exec?sql=update+pbcpgbinfo+set+f_fh_flag%3D1+where+f_txm+in+(" + productStr
        + ")&token=";
    FastHttp.ajaxGet(url, new AjaxCallBack() {
   ...
   * f_cus_no, f_zx_date,f_chk_flag) Select
   * [箱条码],[车号],[线路],[客户],[摊位],[装配日期],[数量],[重量],1)
   * */
  public void insertZxMain(Context mContext,final Handler handler, String cartonCode, String carNo, String dspNo, String cust,
      String cusNo, String zxDate, String qtyNo, String weight, String flag) {
    // String url = StaticField.HOST
    // +
    ...
    try {
      ...
      // Date date=(Date) dateFormat.parse(zxDate);
      // Log.i(TAG, "r:::::"+date);
      url =DbIp.getInstance(mContext).selectById()
          + "exec?sql=insert+into+un_zxmain(f_zx_txm%2Cf_zcd_no%2C+f_zc_dsp%2C+f_ywy%2C+f_cus_no%2C+f_zx_date%2Cf_sum_qty%2Cf_sum_wei%2Cf_chk_flag)%0A"
          + "Select+" + "%27" + cartonCode + "%27%2C%27" + carNo + "%27%2C%27" +URLEncoder.encode(dspNo, "UTF-8")  + "%27%2C%27" + cust
          + "%27%2C%27" + cusNo + "%27%2C%27" + zxDate + "%27%2C%27" + qtyNo + "%27%2C%27" + weight + "%27%2C" + 1
          + "&token=";
      Log.i(TAG, " 请求主表:" + url);
    } catch (Exception e) {
      Log.e(TAG, "", e);
    ...
    FastHttp.ajaxGet(url, config, new AjaxCallBack() {
      ...
      public void callBack(ResponseEntity status) {
        switch (status.getStatus()) {
          case FastHttp.result_ok:
            String json=status.getContentAsString();
            Log.i(TAG, "主表:" +json );
            Message msgSucce=Message.obtain();
            msgSucce.arg1=110;
            msgSucce.obj=json;
            handler.sendMessage(msgSucce);
            break;
          case FastHttp.result_net_err:
            Message msg = Message.obtain();
            msg.arg1 = 0;
